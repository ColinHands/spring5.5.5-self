 ...
 *
 * 通过生成一个与Spring容器交互的CGLIB子类来增强{@link Configuration}类，以尊重{@code @Bean}方法的bean作用域语义。每个这样的{@code @Bean}方法都会在生成的子类中被覆盖，只有当容器实际请求构造一个新实例时才会委托给实际的{@code @Bean}方法实现。否则，对这样的{@code @Bean}方法的调用将作为对容器的引用，通过名称获得相应的bean。
 * @author Chris Beams
...
class ConfigurationClassEnhancer {
	 ...
	 * CGLIB的DefaultGeneratorStrategy的自定义扩展，引入了{@link BeanFactory}字段。还在类生成时将应用程序类加载器公开为线程上下文类加载器(以便ASM在进行通用超类解析时拾取它)。
	 */
