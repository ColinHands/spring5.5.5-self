...
public class AnnotationConfigApplicationContextTests {
	...
	private static class ResourceAnnotatedTestBean extends org.springframework.tests.sample.beans.TestBean {
	...
	public void testPrototypeCreationWithOverriddenResourcePropertiesIsFastEnough() {
		...
		ctx.registerBeanDefinition("spouse", new RootBeanDefinition(org.springframework.tests.sample.beans.TestBean.class));
		org.springframework.tests.sample.beans.TestBean spouse = (org.springframework.tests.sample.beans.TestBean) ctx.getBean("spouse");
		StopWatch sw = new StopWatch();
		sw.start("prototype");
		for (int i = 0; i < 100000; i++) {
			org.springframework.tests.sample.beans.TestBean tb = (org.springframework.tests.sample.beans.TestBean) ctx.getBean("test");
